...
class RuleDrivenRolloutCollector(object):
    ...
    def _environment_action(agent_type, action):
        ...
        return int(values[0])
    @staticmethod
    def _actor_trajectory_assigned(agent_type, agent_state):
        """Only controlling recon leaders produce recon Actor trajectories."""
        if str(agent_type) != "recon":
            return True
        obs = agent_state.get("obs_by_name", {})
        return float(obs.get("is_leader", 0.0)) > 0.5
            ...
            for agent_type in self.bottom_agent_types:
                ...
                for entity_name, agent_state in state.get("agents", {}).items():
                    ...
                    rollout.bottom[agent_type].append(
                        ...
                        step_id=step_id,
                        assigned=self._actor_trajectory_assigned(agent_type, agent_state),
                    )
