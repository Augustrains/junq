...
def create_env(_side_name=None):
    env = AFSIMIslandEnv(auto_start_warlock=True)
    env.wait_for_platforms(["red_recon_1", "red_attack_1", "red_transport_1"], timeout=45.0)
    return env
    ...
    def run(self):
        ...
        while True:
            ...
            if updates % self._print_interval == 0:
                ...
                # === add reward summary for TensorBoard ===
                reward_summary = tf.Summary(value=[
                    tf.Summary.Value(tag="train/avg_reward", simple_value=float(avg_reward)),
                ])
                self._model.writer.add_summary(reward_summary, updates)
