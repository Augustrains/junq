...
class AFSIMRLInterface(object):
    ...
    def get_agent_specs(self) -> Dict[str, dict]:
        ...
        specs = {
            ...
            "landing": {
                ...
                "action_dim": len(self.env.get_landing_action_table()),
    ...
    def _compute_recon_attack_local_rewards(
        ...
        for (group_id, fixed_team_id), members in sorted(groups.items()):
            ...
            if agent_type == "recon":
                ...
                priority_normalizer = max(
                    1.0, float(priority_rules.get("recon_priority_normalizer", 400.0))
                )
                priority_bonus_scale = max(
                    0.0, float(priority_rules.get("recon_priority_bonus_scale", 0.0))
                )
                priority_multiplier += priority_bonus_scale * max(
            ...
            if agent_type == "recon":
                group = self.env.recon_controller.active_groups.get(group_id)
                leader = (
                    self.env.recon_controller.ensure_group_leader(group)
                    if group is not None else None
                )
                reward_members = (
                    [leader.name]
                    if leader is not None and leader.name in members else []
                )
            for name in reward_members:
                self._add_local_detail(
                    local_rewards,
                    details,
                    name,
        ...
        if callable(contributors_for):
            for event in events:
                ...
                contributors = contributors_for(target_name).get("all", [])
                combat_value = getattr(self.env.reward_manager, "_combat_event_value", lambda _event: None)(event)
                if combat_value is None:
                    continue
                value = float(combat_value)
                reward_type = (
                    "contributed_target_hp_loss"
                    if event_type == "damage_dealt" else "contributed_target_destroyed"
                )
                for name in contributors:
                    if name in local_rewards:
                        self._add_local_detail(
                            local_rewards, details, name, reward_type, value, event=event
                        )

        if agent_type == "attack":
            reward_rules = getattr(
                getattr(self.env, "reward_manager", None), "rules", {}
            ).get("attack_return_rewards", {})
            if bool(reward_rules.get("enabled", True)):
                progress_per_10km = float(reward_rules.get("progress_per_10km", 0.5))
                max_progress_m = max(
                    0.0, float(reward_rules.get("max_progress_m_per_step", 50000.0))
                )
                arrival_reward = float(reward_rules.get("arrival_at_carrier", 6.0))
                rearm_base = float(reward_rules.get("rearm_completed_base", 8.0))
                rearm_per_missile = float(
                    reward_rules.get("rearm_completed_per_missile", 4.0)
                )
                rearmed_events = {
                    str(event.get("platform", "")): event
                    for event in events
                    if str(event.get("type", "")) == "attack_rearmed"
                }
                for name in sorted(local_rewards):
                    before_obs = self._obs_by_name(before_entities.get(name, {}))
                    after_obs = self._obs_by_name(after_entities.get(name, {}))
                    ammo_before = (
                        float(before_obs.get("aam_count_norm", 0.0))
                        + float(before_obs.get("agm_count_norm", 0.0))
                    )
                    ammo_after = (
                        float(after_obs.get("aam_count_norm", 0.0))
                        + float(after_obs.get("agm_count_norm", 0.0))
                    )
                    weapon_expended = ammo_before < 2.0 or ammo_after < 2.0
                    returning_before = float(
                        before_obs.get("returning_to_carrier", 0.0)
                    ) > 0.5
                    returning_after = float(
                        after_obs.get("returning_to_carrier", 0.0)
                    ) > 0.5
                    rearming_before = float(before_obs.get("rearming", 0.0)) > 0.5
                    rearming_after = float(after_obs.get("rearming", 0.0)) > 0.5

                    # Dense shaping starts only after a valid RETURN_HOME has
                    # placed this aircraft in the returning state.
                    if weapon_expended and (returning_before or returning_after):
                        before_pos = self._position_from_normalized_obs(before_obs)
                        after_pos = self._position_from_normalized_obs(after_obs)
                        before_carrier = self._position_from_normalized_obs({
                            "lat_norm": before_obs.get("friendly_carrier_lat_norm"),
                            "lon_norm": before_obs.get("friendly_carrier_lon_norm"),
                        })
                        after_carrier = self._position_from_normalized_obs({
                            "lat_norm": after_obs.get("friendly_carrier_lat_norm"),
                            "lon_norm": after_obs.get("friendly_carrier_lon_norm"),
                        })
                        if all(
                            item is not None
                            for item in (before_pos, after_pos, before_carrier, after_carrier)
                        ):
                            before_distance, _ = self.env._distance_and_bearing(
                                before_pos[0], before_pos[1],
                                before_carrier[0], before_carrier[1],
                            )
                            after_distance, _ = self.env._distance_and_bearing(
                                after_pos[0], after_pos[1],
                                after_carrier[0], after_carrier[1],
                            )
                            progress_m = max(
                                0.0,
                                min(max_progress_m, before_distance - after_distance),
                            )
                            if progress_m > 0.0:
                                self._add_local_detail(
                                    local_rewards, details, name,
                                    "attack_return_carrier_progress",
                                    progress_per_10km * progress_m / 10000.0,
                                )

                    if weapon_expended and returning_before and rearming_after:
                        self._add_local_detail(
                            local_rewards, details, name,
                            "attack_return_arrived_at_carrier", arrival_reward,
                        )

                    rearmed_event = rearmed_events.get(name)
                    if rearmed_event is not None:
                        loaded = max(
                            0.0,
                            float(rearmed_event.get("aam_loaded", 0.0))
                            + float(rearmed_event.get("agm_loaded", 0.0)),
                        )
                        if loaded > 0.0:
                            self._add_local_detail(
                                local_rewards, details, name,
                                "attack_return_rearm_completed",
                                rearm_base + rearm_per_missile * loaded,
                                event=rearmed_event,
                            )
        return local_rewards, details
    ...
    def _event_local_reward(self, event: Mapping[str, object]) -> float:
        ...
        if event_type == "damage_dealt":
            combat_value = getattr(self.env.reward_manager, "_combat_event_value", lambda _event: None)(event)
            return 0.0 if combat_value is None else float(combat_value)
