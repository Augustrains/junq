...
class LandingAgent(object):
    ...
    def step(self, group_id: str) -> Dict[str, object]:
        ...
        for ship_name, ship_state in state.get("ships", {}).items():
            action_id = self.select_action(ship_state)
            ok = self.env.apply_landing_ship_action(group_id, ship_name, action_id)
    ...
    def select_action(self, ship_state: Dict[str, object]) -> int:
        mask = list(ship_state.get("action_mask", []))
