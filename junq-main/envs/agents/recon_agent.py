...
class ReconAgent(object):
    """Reconnaissance sub-agent wrapper.

    The high-level commander starts a RECON area task. This agent is responsible
    for reading the per-aircraft recon state, selecting a tactical path action
    for each assigned aircraft, and sending those actions through the env.
    """
    ...
    def step(self, group_id: str) -> Dict[str, object]:
        ...
        for aircraft_name, aircraft_state in state.get("aircraft", {}).items():
            ...
            ok = self.env.apply_recon_aircraft_path_action(group_id, aircraft_name, action_id)
