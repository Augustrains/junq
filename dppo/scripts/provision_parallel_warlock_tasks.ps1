param(
    ...
    [int]$BasePort = 50050,
    [double]$ClockRate = 60,
    [double]$StateUpdateInterval = 15,
    [double]$DecisionPauseInterval = 72,
    [string]$UdpAddress = "10.184.17.133",
...
if ($DecisionPauseInterval -lt 0) { throw "DecisionPauseInterval must be non-negative" }
...
for ($worker = 0; $worker -lt $Workers; $worker++) {
    ...
    $realtimePattern = '(?m)^\s*realtime\s*$'
    $realtimeMatches = [regex]::Matches($rewritten, $realtimePattern)
    if ($realtimeMatches.Count -gt 1) { throw "Expected at most one realtime directive, found $($realtimeMatches.Count)" }
    if ($realtimeMatches.Count -eq 0) {
        $rewritten = $rewritten.Replace("clock_rate $ClockRate", "clock_rate $ClockRate`r`nrealtime")
    }
    ...
    Write-Host "$verb $taskName -> $scenarioName -> UDP ${UdpAddress}:$port clock_rate=$ClockRate state_update_interval=${StateUpdateInterval}s decision_pause_interval=${DecisionPauseInterval}s runner=$Runner"
}
