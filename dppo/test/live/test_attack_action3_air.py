...
def main():
    cfg = str(Path.cwd() / "test" / "configs" / "afsim_units_action3_aam_test.json")
    env = AFSIMIslandEnv(config_path=cfg, bind=True, auto_start_warlock=True)
    try:
        ready = env.wait_for_platforms(["red_attack_1", "blue_attack_2"], timeout=30)
        print("ready", ready, flush=True)
        if not ready:
            return 1
        target = env.platforms["blue_attack_2"]
        env.detected_targets["blue_attack_2"] = {
        ...
        for team_index in range(4):
            group = env.start_attack_group("blue_attack_2", team_index)
        ...
        if spec.get("target_name") != "blue_attack_2":
        ...
        for step in range(22):
            ...
            target = env.platforms["blue_attack_2"]
            slant = env._slant_distance_and_bearing(
            ...
            print(json.dumps({
                ...
                "slant_distance_m": round(slant, 1),
