...
class HAPPOConfig:
    ...
    use_relu: bool = True
    gain: float = 0.01
    share_policy_by_type: bool = False
...
class HAPPOTrainer:
    ...
    def __init__(self, specs: Mapping[str, Mapping[str, object]], global_state_dim: int,
        ...
        for agent_type in self.agent_types:
            ...
            physical_names = tuple(
                str(x) for x in spec.get("entity_names", ()) if str(x)
            ) or (agent_type,)
            names = (agent_type,) if self.config.share_policy_by_type else physical_names
    ...
    def _entity_batches(self, agent_type, batch):
        if self.config.share_policy_by_type:
            # Every physical entity of this type sampled from the same policy,
            # so its rows form one valid on-policy update batch.
            yield self.entity_names[agent_type][0], batch
            return
        names = np.asarray(batch.get("entity_name", []), dtype=object)
        if names.size == 0:
            yield self.entity_names[agent_type][0], batch; return
        for entity_name in self.entity_names[agent_type]:
