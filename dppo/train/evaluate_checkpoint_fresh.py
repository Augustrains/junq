...
def parse_args():
    ...
    parser.add_argument("--max-consecutive-technical-failures", type=int, default=3)
    ...
    parser.add_argument("--evaluation-only", action="store_true", help="run fresh evaluations from --resume without collecting training samples or updating the policy")
    parser.add_argument("--warlock-ssh-target", required=True)
...
def accumulate_attack_target_selections(summary, by_role, by_target, by_phase):
    """Aggregate non-HOLD target choices from one rollout summary."""
    for selection in summary.get("attack_target_selections", []):
        role = str(selection.get("target_role", "unknown") or "unknown")
        target = str(selection.get("target", "unknown") or "unknown")
        phase = str(selection.get("phase", "unknown") or "unknown")
        by_role[role] += 1
        by_target[target] += 1
        by_phase[phase] += 1

def accumulate_attack_execution_diagnostics(summary, diagnostics):
    """Track pursuit convergence, track error, retargets, launches and losses."""
    last_by_aircraft = diagnostics.setdefault("last_by_aircraft", {})
    by_aircraft = diagnostics.setdefault("by_aircraft", {})
    by_role = diagnostics.setdefault("by_role", {})
    deaths = diagnostics.setdefault("red_attack_losses", [])
    for event in summary.get("attack_target_selections", []):
        aircraft = str(event.get("platform", "unknown") or "unknown")
        target = str(event.get("target", "unknown") or "unknown")
        role = str(event.get("target_role", "unknown") or "unknown")
        phase = str(event.get("phase", "unknown") or "unknown")
        actual_distance = float(event.get("actual_horizontal_distance_m", 0.0))
        track_error = float(event.get("track_error_m", 0.0))
        in_range = bool(event.get("in_weapon_range", False))
        role_stats = by_role.setdefault(role, {
            "selections": 0, "fires": 0, "inside_launch_range": 0,
            "actual_distance_sum_m": 0.0, "minimum_actual_distance_m": None,
            "track_error_sum_m": 0.0, "maximum_track_error_m": 0.0,
        })
        role_stats["selections"] += 1
        role_stats["fires"] += int(phase == "fire")
        role_stats["inside_launch_range"] += int(in_range)
        role_stats["actual_distance_sum_m"] += actual_distance
        role_stats["track_error_sum_m"] += track_error
        role_stats["maximum_track_error_m"] = max(role_stats["maximum_track_error_m"], track_error)
        previous_min = role_stats["minimum_actual_distance_m"]
        role_stats["minimum_actual_distance_m"] = actual_distance if previous_min is None else min(previous_min, actual_distance)

        aircraft_stats = by_aircraft.setdefault(aircraft, {
            "selections": 0, "fires": 0, "target_changes": 0,
            "same_target_updates": 0, "same_target_closing": 0,
            "same_target_diverging": 0, "minimum_actual_distance_m": None,
            "last_target": "", "last_actual_distance_m": None,
        })
        aircraft_stats["selections"] += 1
        aircraft_stats["fires"] += int(phase == "fire")
        previous = last_by_aircraft.get(aircraft)
        if previous is not None:
            if previous["target"] != target:
                aircraft_stats["target_changes"] += 1
            else:
                aircraft_stats["same_target_updates"] += 1
                if actual_distance < previous["distance_m"]:
                    aircraft_stats["same_target_closing"] += 1
                elif actual_distance > previous["distance_m"]:
                    aircraft_stats["same_target_diverging"] += 1
        previous_min = aircraft_stats["minimum_actual_distance_m"]
        aircraft_stats["minimum_actual_distance_m"] = actual_distance if previous_min is None else min(previous_min, actual_distance)
        aircraft_stats["last_target"] = target
        aircraft_stats["last_actual_distance_m"] = actual_distance
        last_by_aircraft[aircraft] = {"target": target, "distance_m": actual_distance}

    for event in summary.get("combat_events", []):
        if str(event.get("side", "")) == "red" and str(event.get("role", "")) == "attack_aircraft":
            name = str(event.get("platform", "unknown"))
            if not any(item.get("platform") == name for item in deaths):
                deaths.append({
                    "platform": name,
                    "sim_time": float(event.get("sim_time", 0.0)),
                    "shooter": str(event.get("actor", "")),
                })


def finalize_attack_execution_diagnostics(diagnostics):
    by_role = {}
    for role, raw in sorted(diagnostics.get("by_role", {}).items()):
        count = max(1, int(raw.get("selections", 0)))
        by_role[role] = {
            "selections": int(raw.get("selections", 0)),
            "fires": int(raw.get("fires", 0)),
            "inside_launch_range": int(raw.get("inside_launch_range", 0)),
            "mean_actual_distance_m": round(float(raw.get("actual_distance_sum_m", 0.0)) / count, 3),
            "minimum_actual_distance_m": round(float(raw.get("minimum_actual_distance_m") or 0.0), 3),
            "mean_track_error_m": round(float(raw.get("track_error_sum_m", 0.0)) / count, 3),
            "maximum_track_error_m": round(float(raw.get("maximum_track_error_m", 0.0)), 3),
        }
    by_aircraft = {}
    for name, raw in sorted(diagnostics.get("by_aircraft", {}).items()):
        by_aircraft[name] = {
    ...
    return {
        ...
        "red_attack_losses": sorted(
            diagnostics.get("red_attack_losses", []),
            key=lambda item: (float(item.get("sim_time", 0.0)), str(item.get("platform", ""))),
        ),
    }
...
TECHNICAL_TERMINATION_REASONS = {
    "udp_state_stream_stalled",
    "native_decision_pause_timeout",
    "native_pause_timeout",
    "worker_startup_failed",
}


def trajectory_quality(worker, args, reason, steps):
    """Return whether an episode is policy-valid rather than technically cut off."""
    if reason in TECHNICAL_TERMINATION_REASONS:
        return False, reason
    expected_steps = int(args.max_episode_steps)
    if reason in ("landing_deadline_missed", "stage_time_limit") and int(steps) < expected_steps:
        return False, "incomplete_{0}_{1}_of_{2}_steps".format(
            reason, int(steps), expected_steps
        )
    # Early combat landing and genuine scenario terminals are meaningful PPO
    # outcomes even when they end before the three-hour horizon.
    return True, "policy_terminal_{0}".format(reason)

def native_pause_state_stream_stalled(env, args):
    """Only declare a state failure when both data and native control are stale.

    At a native boundary DecisionReady is deliberately transmitted before the
    full state snapshot.  Under a short UDP burst the control packet can arrive
    first; give the batch a small drain window and keep sampling if subsequent
    control heartbeats prove the scenario is still alive.
    """
    age = env.platform_state_age_seconds()
    if age <= args.platform_state_stall_seconds:
        return False
    if not args.native_decision_pause:
        return True
    control_age_reader = getattr(env, "decision_ready_age_seconds", None)
    control_age = control_age_reader() if callable(control_age_reader) else float("inf")
    if control_age > max(2.0, float(env.message_timeout_seconds) * 4.0):
        return True
    env._drain_messages(timeout=min(0.5, max(0.1, float(env.message_timeout_seconds) * 2.0)))
    if env.platform_state_age_seconds() <= args.platform_state_stall_seconds:
        return False
    print(
        "native_pause_state_snapshot_delayed",
        env.local_address[1],
        "state_age", round(age, 3),
        "control_age", round(control_age, 3),
        "sim", round(env._current_sim_time(), 3),
        flush=True,
    )
    return False
...
def run_evaluation(args, worker, trainer, update_id, training_steps, training_episodes, evaluation_episode=1):
    ...
    reuse_running = os.environ.get("EVALUATION_REUSE_RUNNING_SCENARIO", "").lower() in ("1", "true", "yes", "on")
    if reuse_running:
    ...
    evaluation_steps = 0
    attack_target_roles = Counter()
    attack_target_names = Counter()
    attack_target_phases = Counter()
    attack_execution_diagnostics = {}
    status = high_quality_landing_status(
        env, args.min_recon_alive, args.min_attack_alive, args.min_loaded_transports
    )
    evaluation_error = ""
    for step in range(1, int(args.eval_max_episode_steps) + 1):
        try:
            ...
            if "AFSIM native decision pause timed out" not in evaluation_error:
                raise
            reason = "native_pause_timeout"
            print(
                "evaluation_technical_failure", worker["id"],
                "episode", evaluation_episode, "error", evaluation_error,
                flush=True,
            )
            break
        summary = rollout.summary()
        evaluation_steps += int(summary.get("steps", 0))
        accumulate_attack_target_selections(summary, attack_target_roles, attack_target_names, attack_target_phases)
        accumulate_attack_execution_diagnostics(summary, attack_execution_diagnostics)
        team_reward += float(summary.get("team_reward_sum", 0.0))
        if native_pause_state_stream_stalled(env, args):
            status = high_quality_landing_status(
        ...
        if current_reason:
            ...
            break
    bonus = 0.0 if reason in TECHNICAL_TERMINATION_REASONS else terminal_bonus(env, status, reason, args)
    result = {
        ...
        "done_reason": reason,
        "technical_error": evaluation_error,
        ...
        "attack_target_selection_counts": dict(sorted(attack_target_roles.items())),
        "attack_target_selection_targets": dict(sorted(attack_target_names.items())),
        "attack_target_selection_phases": dict(sorted(attack_target_phases.items())),
        "attack_execution_diagnostics": finalize_attack_execution_diagnostics(attack_execution_diagnostics),
...
def main():
    ...
    if args.resume:
        ...
        print("resume_loaded", True, "path", args.resume, flush=True)
    ...
    if update_id >= args.updates and not args.evaluation_only:
        print("resume_target_already_reached", "checkpoint_update", update_id, "target_updates", args.updates, flush=True)
        return 0
    ...
    try:
        if args.evaluation_only:
            ...
            evaluations = [future.result() for future in evaluation_futures]
            evaluations.sort(key=lambda row: int(row["evaluation_episode"]))
            for evaluation in evaluations:
                append_metrics_jsonl(metrics_file, evaluation)
            print("EVALUATION_ONLY_COMPLETE", json.dumps(evaluations, sort_keys=True), flush=True)
            return 0
        ...
        while update_id < args.updates and any(
            ...
            round_attack_target_roles = {worker["id"]: Counter() for worker in active}
            round_attack_target_names = {worker["id"]: Counter() for worker in active}
            round_attack_target_phases = {worker["id"]: Counter() for worker in active}

            for step in range(1, args.max_episode_steps + 1):
                sampling = collect_parallel(executor, active, done, trainer, return_errors=True)
                for worker in active:
                    worker_id = worker["id"]
                    if worker_id in done:
                        continue
                    result = sampling[worker_id]
                    collection_error = ""
                    if isinstance(result, dict) and "_collection_error" in result:
                        collection_error = str(result["_collection_error"])
                        if "AFSIM native decision pause timed out" not in collection_error:
                            raise RuntimeError(
                                "worker {0} unexpected rollout exception: {1}".format(
                                    worker_id, collection_error
                                )
                            )
                        # A native-pause timeout occurs before a rollout can
                        # be returned. Treat it as a discarded technical
                        # episode, not a fatal error for the whole learner.
                        summary = {}
                        reason = "native_pause_timeout"
                        status = high_quality_landing_status(
                        ...
                        print("worker_collection_technical_failure", worker_id,
                              "reason", reason, "error", collection_error, flush=True)
                    else:
                        rollout = result
                        summary = rollout.summary()
                        ...
                        accumulate_attack_target_selections(
                            summary, round_attack_target_roles[worker_id],
                            round_attack_target_names[worker_id], round_attack_target_phases[worker_id],
                        )
                        if native_pause_state_stream_stalled(worker["env"], args):
                            status = high_quality_landing_status(
                                worker["env"], args.min_recon_alive,
                                args.min_attack_alive, args.min_loaded_transports,
                            )
                    ...
                    if not reason:
                        continue
                    episode_steps = int(round_steps[worker_id])
                    worker["attempt"] += 1
                    trajectory_valid, quality_reason = trajectory_quality(
                        worker, args, reason, episode_steps
                    )
                    if not trajectory_valid:
                        # Technical cutoffs contain a partial, on-policy prefix
                        # without its intended terminal outcome.  Never merge it.
                        worker["consecutive_technical_failures"] += 1
                        training_steps -= episode_steps
                        discarded_samples = worker["buffer"].assigned_counts()
                        worker["buffer"].discard_episode()
                        done.add(worker_id)
                        append_metrics_jsonl(metrics_file, {
                            "record_type": "training_episode_discarded",
                            "worker": worker_id,
                            "worker_attempt": int(worker["attempt"]),
                            "valid_training_episodes": int(training_episodes),
                            "training_steps": int(training_steps),
                            "policy_update": int(update_id),
                            "steps": episode_steps,
                            "expected_decision_steps": int(args.max_episode_steps),
                            "done_reason": reason,
                            "trajectory_valid": False,
                            "trajectory_quality_reason": quality_reason,
                            "consecutive_technical_failures": int(worker["consecutive_technical_failures"]),
                            "sim_seconds": float(worker["env"]._current_sim_time()),
                            "episode_start_decision_time": float(worker.get("episode_start_decision_time", 0.0)),
                            "episode_end_decision_time": float(worker["env"].native_decision_ready_time),
                            "discarded_samples": discarded_samples,
                            "collection_error": collection_error,
                        })
                        print(
                            "worker_trajectory_discarded", worker_id,
                            "attempt", worker["attempt"], "reason", quality_reason,
                            "steps", episode_steps,
                            "consecutive_failures", worker["consecutive_technical_failures"],
                            flush=True,
                        )
                        # A single Windows/Mission/UDP worker can temporarily
                        # lose its native-pause state stream. Its partial
                        # trajectory is already discarded above; do not abort
                        # the shared learner or the healthy workers. The
                        # normal restart block at the end of this round gives
                        # this worker a clean scenario and UDP session.
                        if worker["consecutive_technical_failures"] >= args.max_consecutive_technical_failures:
                            print(
                                "worker_technical_failure_recovery", worker_id,
                                "consecutive_failures", worker["consecutive_technical_failures"],
                                "action", "discard_and_relaunch_worker",
                                flush=True,
                            )
                        continue

                    worker["consecutive_technical_failures"] = 0
                    ...
                    append_metrics_jsonl(metrics_file, {
                        ...
                        "expected_decision_steps": int(args.max_episode_steps),
                        "moved_samples": moved,
                        "attack_target_selection_counts": dict(sorted(round_attack_target_roles[worker_id].items())),
                        "attack_target_selection_targets": dict(sorted(round_attack_target_names[worker_id].items())),
                        "attack_target_selection_phases": dict(sorted(round_attack_target_phases[worker_id].items())),
                    })
                    print(
                        "worker_trajectory_merged", worker_id,
                        "episode", worker["episode"], "attempt", worker["attempt"],
                        "reason", reason, "steps", episode_steps, "moved", moved,
            ...
            if global_buffer.ready(minimum):
                ...
                # Persist immediately after the optimizer update.  Evaluation
                # starts fresh Mission tasks and is external I/O; a startup
                # failure must not discard a valid shared-policy update.
                ...
                _, pre_evaluation_latest = save_training_checkpoint(
                    pre_evaluation_payload, checkpoint_dir,
                    "bottom_happo_recon_attack_parallel_eval", update_id,
                )
                print("parallel_eval_checkpoint_pre_evaluation", pre_evaluation_latest, flush=True)
                evaluation_workers = [
                    workers[(args.eval_worker + offset) % args.workers]
                    for offset in range(int(args.eval_episodes))
                ]
