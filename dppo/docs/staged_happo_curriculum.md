...
## Official HAPPO implementation

The bottom trainer uses vendored `marlbenchmark/on-policy` components:

- `HAPPO_Policy`: upstream actor, critic, and optimizers.
- `HAPPO`: upstream clipped loss and sequential factor update.
- `SeparatedReplayBuffer`: upstream minibatch generator.

AFSIM only adapts observations, masks, and trajectories. Each persistent unit has
an independent policy. Legacy custom-HAPPO checkpoints are rejected.
高层课程指示器只能下发当前阶段允许的任务。

## Negative reward switch

Negative reward terms are disabled by default. Positive event, progress, and
terminal rewards are unchanged. Suppressed terms remain visible in reward
diagnostics with `raw_value` and `negative_reward_suppressed=true`.

Enable all negative terms explicitly with:

```bash
ENABLE_NEGATIVE_REWARDS=1 bash scripts/linux_train_bottom_mappo.sh
```

For direct Python launches, use `--enable-negative-rewards`.
