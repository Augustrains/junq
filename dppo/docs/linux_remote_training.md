...
The current island scenario includes:
...
Bottom-policy frequency is configured in simulation time. The default is:

```bash
BOTTOM_DECISIONS_PER_HOUR=50
SIMULATION_CLOCK_RATE=40
```

This gives one decision every 72 simulation seconds. At `clock_rate 40`, the
training process waits 1.8 wall-clock seconds between decisions:

```text
3600 / 50 = 72 simulation seconds
72 / 40 = 1.8 wall-clock seconds
```

If the scenario clock rate changes, set `SIMULATION_CLOCK_RATE` to the same
value. `DECISION_SECONDS` remains available only as an explicit wall-clock
override; leaving it at `0` enables automatic conversion.
