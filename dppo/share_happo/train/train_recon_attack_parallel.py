...
def task_command(args,worker,action):
 ...
 if action=='start': body="Start-ScheduledTask -TaskName '{0}'".format(task)
 else: body="Stop-ScheduledTask -TaskName '{0}' -ErrorAction SilentlyContinue".format(task)
...
def collect_parallel(executor, active_workers, done_worker_ids, trainer):
