...
ARGS=("${TRAIN_SCRIPT}" --workers "${PARALLEL_WORKERS}" --base-port "${BASE_UDP_PORT}" --device "${DEVICE}" --updates "${TARGET_UPDATES}" --episodes-per-worker "${TRAIN_EPISODES}" --max-episode-steps "${MAX_EPISODE_STEPS}" --recon-min-samples "${RECON_MIN_SAMPLES}" --attack-min-samples "${ATTACK_MIN_SAMPLES}" --update-epochs "${UPDATE_EPOCHS}" --minibatch-size "${MINIBATCH_SIZE}" --bottom-decisions-per-hour "${BOTTOM_DECISIONS_PER_HOUR}" --simulation-clock-rate "${SIMULATION_CLOCK_RATE}" --decision-seconds "${DECISION_SECONDS}" --platform-state-stall-seconds "${PLATFORM_STATE_STALL_SECONDS}" --bottom-global-reward-weight "${BOTTOM_GLOBAL_REWARD_WEIGHT}" --bottom-global-reward-clip "${BOTTOM_GLOBAL_REWARD_CLIP}" --deadline-miss-penalty "${RECON_ATTACK_DEADLINE_MISS_PENALTY}" --checkpoint-dir "${CHECKPOINT_DIR}" --metrics-file "${METRICS_FILE}" --warlock-ssh-target "${WINDOWS_SSH_TARGET}" --warlock-ssh-port "${WINDOWS_SSH_PORT}" --warlock-ssh-key "${WINDOWS_SSH_KEY}" --warlock-task-prefix "${WINDOWS_TASK_PREFIX}")
...
echo "[parallel_happo] workers=${PARALLEL_WORKERS} ports=${BASE_UDP_PORT}..$((BASE_UDP_PORT+PARALLEL_WORKERS-1)) shared_checkpoint=${CHECKPOINT_DIR}"
